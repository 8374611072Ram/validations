from wsgiref.validate import validator
from django import forms 
from django.core import validators


def check1_a(value):
    if value[0]=='a':
        raise forms.ValidationError('start without a')

def check2_len(v):
    if len(v)<=5:
        raise forms.ValidationError('length issue')

class StudentForm(forms.Form):
    name=forms.CharField(max_length=100,validators=[check1_a,validators.MaxLengthValidator])
    botcatcher=forms.CharField(max_length=100,widget=forms.HiddenInput,required=False)
    email=forms.CharField(max_length=100)
    reenter_email=forms.CharField(max_length=100)
    mobile=forms.CharField(max_length=10,min_length=10, validators=[validators.RegexValidator('[6-9]\d{9}')])


    def clean_botcatches(self):
        data=self.cleaned_data['botcatcher']
        if len(data)<=0:
            raise forms.ValidationError('bot')

    def clean(self):
        e=self.cleaned_data['email']
        r=self.cleaned_data['reenter_email']
        if e!=r:
            raise forms.ValidationError('Incorrect email')